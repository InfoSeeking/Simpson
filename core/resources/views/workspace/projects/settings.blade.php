...
<div class="row">
    ...
    <div class="col-md-8">
		...
		<form id='project-description'>
			...
			<div class='form-group'>
				<textarea class='form-control' name='scores' placeholder='Score JSON'>{{$project->scores}}</textarea>
			</div>
			<div class='form-group'>
...
(function(){
	...
	$('#project-description').on('submit', function(e){
		...
		var scores = $(this).find('textarea[name=scores]').val();
		$.ajax({
			url: '/api/v1/projects/' + projectId,
			method: 'put',
			data: {
			...
			success: function() {
				MessageDisplay.display(['Project information updated.'], 'success');
			},
			error: function(xhr) {
				var json = JSON.parse(xhr.responseText);
				MessageDisplay.displayIfError(json);
